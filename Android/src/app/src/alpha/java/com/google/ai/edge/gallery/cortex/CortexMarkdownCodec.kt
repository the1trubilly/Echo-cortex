...
internal object CortexMarkdownCodec {
  ...
  fun encodeNormalizationSidecar(
    ...
    buildString {
        ...
        appendLine("> This file does not replace, confirm, or supersede its exact source.")
        appendLine("> Its LOD0 preview is deterministic navigation metadata, not source truth.")
        appendLine("> Delete and rebuild it at any time; the source Markdown and hashes remain truth.")
        appendLine()
        appendLine("## LOD0 memory handle")
        appendLine()
        appendLine("- Representative title: ${yaml(metadata.title)}")
        ...
        appendLine()
        appendLine("### Caveman Minimum Viable Nuance")
        appendLine()
        appendLine(metadata.cavemanMvnSummary)
        appendLine()
